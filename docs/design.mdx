...
I have reincarnated JohnTelford.com using several of these and other static site generators [Development Frameworks](https://docusaurus.io/docs#comparison-with-other-tools). This latest reincarnations was buildt using Docusaurus development framework.

The website design prerequisites can most likely be realized using the [Docusaurus](https://docusaurus.io) development framework. Several of the Docusaurus attributes may be changed because `Docusaurus` fabstracts `react` which can be used `mdx` and `jsx` to creating more complex user interfaces .
