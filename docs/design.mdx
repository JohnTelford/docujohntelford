...
I have reincarnated JohnTelford.com using several of these and other static site generators [Development Frameworks](https://docusaurus.io/docs#comparison-with-other-tools). This latest reincarnations was built using Docusaurus development framework.

The website design prerequisites may be realized using the [Docusaurus](https://docusaurus.io) development framework. Several Docusaurus attributes may be edited because `Docusaurus` abstracts `react` which uses `mdx` and `jsx` for creating user interfaces.
