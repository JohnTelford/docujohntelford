...
_Web Development Components For The Component Era._ Web Components are independent and reusable bits of code. They serve the same purpose as [JavaScript](https://developer.mozilla.org/en-US/docs/Web/JavaScript) functions, but work in isolation.
...
The core development technology of [Docusaurus](https://docusaurus.io) and React is [JavaScript](https://developer.mozilla.org/en-US/docs/Web/JavaScript). React has created JSX, a syntax extension to JavaScript. JSX is not part of the web programming languages [Troika](programming#troika). Browsers do not understand JSX. React created a compiler to transform JSX code into standard JavaScript at build time.
